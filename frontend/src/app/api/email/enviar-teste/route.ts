...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const isKeyMissing = e instanceof Error && e.message === "ENCRYPTION_KEY não configurada";
      return NextResponse.json(
        {
          error: isKeyMissing
            ? "ENCRYPTION_KEY não configurada. No Vercel: Settings → Environment Variables → adicione ENCRYPTION_KEY com o mesmo valor do seu .env local."
            : "Não foi possível descriptografar o Client Secret. O valor foi criptografado em outro ambiente (ex.: local). Solução: acesse o site na Vercel, vá em Configurações → E-mail, edite esta configuração, cole o Client Secret novamente e salve (assim será criptografado com a chave da Vercel).",
        },
        { status: 500 }
      );
    }
    ...
    const content = (descricao && descricao.trim()) ? descricao.trim() : "E-mail de teste do sistema Recebi$.";
    const sendBody = {
      message: {
